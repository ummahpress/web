// Ummah Press App - Working Version with Folder Structure
...
async function loadPosts() {
    try {
        ...
        return loadedPosts.sort((a, b) => new Date(b.date) - new Date(a.date));
    } catch (error) {
...
async function loadTranslations() {
    try {
        // Try to load translations from single file first
        const response = await fetch('data/translations.json');
        return await response.json();
    } catch (error) {
        console.log('No translations.json found, trying folder structure...');
        ...
        for (const post of posts) {
            ...
            try {
                const langResponse = await fetch(`data/translations/post-${postId}/index.json`);
                const langFiles = await langResponse.json();
...
async function initApp() {
    try {
        ...
        // Initialize mobile menu event listeners FIRST
        setupMobileMenu();
        
        // Load data and render posts
        await loadData();
        setActivePage('home');
